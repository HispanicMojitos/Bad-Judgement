...
using UnityEngine;
// /!\  ATTENTION CE SCRIPT VA BEAUCOUP CHANGER, NOTTEMMENT LORSQU'ON VA METTRE PLUSIEURS ARME, JY AI BEAUCOUP REFLECHI ET JE VERRAI AVEC VOUS CAR IL FAUT ADAPTER LE SCRIPTS EN FONCTION DES ARMES
public class SoundsOfShoots : MonoBehaviour
{
    ...
    private bool finDeShootSound = true; // permet de savoir dans l'algorithme quand la fin du clip "shootSound" est arrivée
    private bool debutDeendsong = false; // permet de savoir dans l'algorithme quand le debut du clip 'endsong' doit avoir lieu
    private bool doitRecharger = false; // permet de savoir dans l'algorithme de jouer le son de rechargement apres le son de tirs, alors que l'on aie appuyé sur la touche de rechargement pendant le tir
	...
	void Update ()
    {
		...
		if(Input.GetButton("Fire1") && finDeShootSound == false)  // Si la touche pour tirer est préssé et que le son doit continuer de jouer (indice1) , on joue le son !! => ALGORITHME
        {
            debutDeendsong = false; // Empeche une boucle infinie du son de fin, croyez moi IL FAUT laisser ca la x) => ALGORITHME
        ...
        else if((Input.GetKeyDown(KeyCode.R) && !machineGun.isPlaying) || (doitRecharger == true && !machineGun.isPlaying)) // Si la touche R est préssée pour tirer, le son de recharge se fait !!
        {
            machineGun.clip = reload; // On implemente le son de rechargement
            machineGun.Play(); // On joue le son de rechargement
            doitRecharger = false; // Puisque le son
        }
        else if (sonJoué == true) // permet d'acceder a la condition suivante et de decrementer le  temps du son actuel du tir
        {
                debutDeendsong = true; // Permet d'acceder a la condition suivante 
                timeSound = timeSound - Time.deltaTime; // Decrementation du temps de son passé
                if (debutDeendsong == true && timeSound > endsong.length) // Permet de jouer le son de fin 
                {
