...
public class AIscripts : MonoBehaviour
{
    ...
    private int distanceDeVueMax = 40; // Distance entre l'IA et le joueur a partir de laquelle l'IA va commencer a suivre le joueur
    private int distanceAttaque = 20;// Distance entre l'IA et le joueur a partir de laquelle l'IA va commencer a attaquer
    ...
    void Update()
    {
        ...
        {// Si la distance entre le joueur  ET l'IA auquel on attache ce script est inférieur à 10, ET que le joueur se trouve dans la région de l'espace situé dans l'angle de vue défini de l'IAalors on va faire quelquechose

            RaycastHit hit; // On utilise un raycast pour voir si l'IA voit le joueurs
            Debug.DrawLine(head.transform.position, direction * 100, Color.red); // permet d'afficher la direction du tir des balles
            if (Physics.Raycast(head.transform.position, direction, out hit)) // Si l'IA voit le joueur
            {
                if (hit.transform.position == Player.position) 
                { // Si la distance entre le premier gameobject auquel le raycast se heurte et l'IA  EST EGALE à la distance entre l'IA et et le joueur (permet de voir si il des objets ou non entre l'IA et le joueur

                    IsPatrolling = false;
                    this.transform.rotation = Quaternion.Slerp(this.transform.rotation, Quaternion.LookRotation(direction), 0.1f);
                    // La rotation de l'IA va se faire en fonction de 2 parametre :                                            
